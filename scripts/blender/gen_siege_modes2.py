...
def gen_spawn_beacon():
    ...
    les fenêtres éclairées) au sommet pour rester visible de loin."""
    stone = mat("pierre", STONE)
    wood_dark = mat("bois_sombre", WOOD_DARK)
    glow = mat("orbe", GLOW_YELLOW, emission=1.8)
    cylinder("Socle", stone, (0, 0, 0.12), radius=0.28, depth=0.24, vertices=10)
    cylinder("Mat", wood_dark, (0, 0, 1.1), radius=0.06, depth=1.75, vertices=8)
    cylinder("Orbe", glow, (0, 0, 2.05), radius=0.16, depth=0.28, vertices=10)
    export("siege_spawn_beacon.glb")
...
def gen_portcullis():
    ...
    intégration future sans re-échelle."""
    metal_dark = mat("metal_sombre", METAL_DARK)
    opening_w, opening_h = 3.6, WALL_H
    n_vert, n_horiz = 8, 5
    for i in range(n_vert):
        x = -opening_w / 2 + 0.2 + i * (opening_w - 0.4) / (n_vert - 1)
        cube(f"Barreau{i}", metal_dark, (x, 0, opening_h / 2), (0.06, 0.10, opening_h))
    for i in range(n_horiz):
        z = 0.2 + i * (opening_h - 0.4) / (n_horiz - 1)
        cube(f"Traverse{i}", metal_dark, (0, 0, z), (opening_w - 0.3, 0.10, 0.06))
    for sx in (-1, 1):

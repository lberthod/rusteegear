...
| **O** — Physique & feel (trimesh/convexe, CCD/couches, requêtes, 103a maintenabilité `app`/`editor`/`scene`, 103b character controller, 103c audit prédiction réseau) | 100 → 103c | 🟢 (100/101/102/103a-1/103a-2 faits ; 103a-3/103b/103c restants) |
...
103a-103c : **103a-1** (découpage de `app/mod.rs`/`AppState` en 7 sous-modules) et
**103a-2** (même traitement pour `editor/mod.rs` et `scene/mod.rs`) sont faits ;
restent **103a-3** (migration des commentaires d'historique vers `docs/audits/`),

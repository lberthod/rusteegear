...
| 7. `sim_step` D | Pilotage physique du joueur (`drive_local_and_networked_players`) | ✅ `764ceda`, [run 33848533899](https://github.com/lberthod/rusteegear/actions/runs/33848533899) 7/7 vert. Sans playtest manuel (décision utilisateur) — 8 tests automatisés ciblés vérifiés (tank controls, gamepad, clip Idle/Walk). `sim_step` ~800 → ~360 lignes cumulé |
| 8. `sim_step` E | Ciblage et poursuite IA (`chase_and_patrol_ai_creatures`, un seul lot plutôt que E1-E3 : la boucle de tri/plafond/mouvement s'est révélée non séparable sans re-complexifier les emprunts `phys`) | ✅ `f5c0914`, [run 33849617855](https://github.com/lberthod/rusteegear/actions/runs/33849617855) 7/7 vert. Sans playtest manuel (décision utilisateur) — 7 tests automatisés ciblés vérifiés, dont `brawl_demo_rival_survives_two_hits_then_falls_on_the_third` (le test de ring-out cité en commentaire dans le code d'origine). **`sim_step` : ~800 → ~215 lignes — plan de découpage terminé (12 lots, 6+6)** |

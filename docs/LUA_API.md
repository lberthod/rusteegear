...
| `vr.active` | booléen | Vrai dans une session **VR** (APK Meta Quest, simulateur `quest_sim`) ; faux ailleurs — un même script marche partout. Table construite seulement si le script mentionne `vr`. |
| `vr.head` | table `{x, y, z, yaw}` ou absente | Tête du joueur en coordonnées **monde** (m) ; `yaw` = lacet du regard (rad, même convention que `obj.ry`). Absente hors VR. |
| `vr.left`, `vr.right` | table `{x, y, z, trigger, grip, a, b}` ou absente | Manette (poignée) en monde ; `trigger`/`grip` dans [0, 1] ; `a`/`b` = A/B à droite, X/Y à gauche. Absente si la manette n'est pas suivie. |
...
| `vr.haptic(côté, intensité, durée)` | — | Fait vibrer la manette `"left"` ou `"right"` (intensité 0..1, durée en s, bornée à 2 s). Sans effet hors VR. Si le jeu vibre déjà (coup encaissé), la plus forte demande de l'image l'emporte. |

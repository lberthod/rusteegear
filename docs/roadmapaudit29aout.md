...
| 2.3 | Regrouper `AppState` en sous-structs par domaine | dette | 🔶 Lot 1/N fait (`6f4cbfd`) : cluster FX/HUD (9 champs cosmétiques — flashs/secousse/bannières) extrait dans `FxState`, 120 sites d'appel corrigés (script sur les suggestions `--message-format=json` du compilateur, pas de sed manuel), 684/684 tests verts. **Ampleur réelle mesurée en route** : ~120-175 champs, 71 fichiers concernés — bien plus gros qu'un simple refactor `L`. Reste par lots séparés (état UI éditeur, réseau, gameplay...) ; `build_ui` (0 test) reste le point le plus risqué à toucher |

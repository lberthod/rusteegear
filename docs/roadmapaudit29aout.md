...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | 🔶 4 lots faits, méthode : chaque lot vérifié par build + 693 tests + capture d'écran réelle (`screencapture`), pas seulement par compilation. Lot 1 (`bdc1b80`) : 3 modales de confirmation → `confirmation_modals(...)`. Lot 2 (`82ac99c`) : fin de manche/overlay tactile/widgets HUD → `end_of_round_and_hud_widgets(...)`. Lot 3 (`a4d3101`) : mini-carte/collectibles/bannières de Play → `play_overlays(...)`. Lot 4 (`5d4eabb`) : barre d'outils complète (lecture/gizmo/undo-redo/save/aperçus/debug/build) → `toolbar(...)` — capture bonus où la modale du lot 1 s'est déclenchée spontanément (autosave détecté), validant les deux extractions ensemble. Le chemin Play lui-même reste non vérifiable au clic : tentative d'automatiser via `osascript`/System Events refusée (permission Accessibilité macOS non accordable par cette session). Code déplacé à l'identique à chaque lot. Reste le gros morceau : bloc panneaux latéraux (~660 l., aperçu mobile/cadre téléphone) — nettement plus couplé, à traiter avec la même prudence |

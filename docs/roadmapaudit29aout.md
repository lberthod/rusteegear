...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | ✅ 5 lots faits (`bdc1b80`, `82ac99c`, `a4d3101`, `5d4eabb`, `5fca41f`) : `confirmation_modals`, `end_of_round_and_hud_widgets`, `play_overlays`, `toolbar`, `inspector_panel` (le plus gros, 636 lignes — panneau Inspecteur complet, propriétés objet/lumière/éclairage/ciel). `build_ui` passe de 1382 à ~427 lignes (hiérarchie déjà déléguée à `hierarchy_panel`, aperçu mobile/cadre téléphone restants). Méthode systématique : chaque lot vérifié par build + clippy + 693 tests + capture d'écran réelle (`screencapture`), pas seulement par compilation — deux captures ont même déclenché spontanément du contenu couvert par les lots précédents (modale autosave, panneau Inspecteur), validant plusieurs extractions à la fois. Un incident de script (attribut clippy mal placé) détecté et corrigé par `cargo clippy` avant tout commit. Le chemin Play lui-même reste non vérifiable au clic (permission Accessibilité macOS non accordable par cette session). Reste l'aperçu mobile/cadre téléphone (~110 l.) si on veut aller plus loin — valeur marginale décroissante à ce stade |

...
| 3.4 | **Build wasm32 cassé** : `network_client.rs:855/862` appelle `check_palier_atteint` depuis un `impl AppState` compilé sur toutes cibles, mais la méthode n'est définie que dans un `impl AppState` gated `#[cfg(not(any(ios, android, wasm32)))]` (ligne 1033) | régression silencieuse | ⏳ Trouvé le 2026-08-29 en vérifiant le lot 13 (interpréteur Lua) sur les deux cibles ; confirmé pré-existant au commit `2e73226`, avant tout changement de cette session — `cargo build --lib --target wasm32-unknown-unknown` était déjà rouge. À vérifier : pourquoi le job CI `cross-build` (wasm32) ne l'a pas détecté |

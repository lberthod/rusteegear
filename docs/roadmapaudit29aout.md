...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | 🔶 Lot 1/N fait (`bdc1b80`) : les 3 modales de confirmation (quitter/fermer projet, restauration autosave) extraites dans `confirmation_modals(...)` — bloc autonome, overlay `egui::Modal`, risque minimal. Vérifié en plus par lancement réel + capture d'écran (`screencapture`), aucune régression visible. Reste le gros du corps (barre d'outils ~208 l., bloc panneaux ~660 l., aperçu mobile, minimap, fin de partie...) — chaque lot suivant devra être vérifié de la même façon (build + tests + capture d'écran), pas seulement par compilation |

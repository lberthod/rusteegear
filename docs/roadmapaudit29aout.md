...
| 2.3 | Regrouper `AppState` en sous-structs par domaine | dette | 🔶 Lot 1/N fait (`6f4cbfd`) : cluster FX/HUD (9 champs cosmétiques) → `FxState`, 120 sites corrigés. Lot 2/N fait (`62695fc`) : panneaux Multijoueur (12 champs chat/leaderboard/online_players) → `NetPanelsState`, 34 sites corrigés — **incident en route** : script de correction rejoué sur un JSON de diagnostics périmé, a corrompu `frame.rs` (offsets appliqués deux fois) ; détecté au diff avant commit, fichier restauré (`git checkout`) et refait proprement. 684/684 tests verts après coup. **Ampleur réelle mesurée** : ~120-175 champs, 71 fichiers concernés — bien plus gros qu'un simple refactor `L`. Reste par lots séparés (état UI éditeur, gameplay, session réseau bas niveau...) ; `build_ui` (0 test) reste le point le plus risqué à toucher |

...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | 🔶 2 lots faits, méthode : chaque lot vérifié par build + 693 tests + capture d'écran réelle (`screencapture`), pas seulement par compilation. Lot 1 (`bdc1b80`) : 3 modales de confirmation → `confirmation_modals(...)`. Lot 2 (`82ac99c`) : fin de manche/overlay tactile/widgets HUD → `end_of_round_and_hud_widgets(...)` — capture faite en mode Édition (rendu intact), le chemin Play lui-même non vérifiable visuellement faute d'automatisation de clic sur fenêtre native ; code déplacé à l'identique, pas de changement de logique. Reste le gros du corps (barre d'outils ~208 l., bloc panneaux ~660 l., aperçu mobile, minimap) — nettement plus couplé, chaque lot suivant à traiter avec la même prudence |

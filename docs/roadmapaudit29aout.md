...
| 2.3 | Regrouper `AppState` en sous-structs par domaine | dette | 🔶 10 lots faits : FX/HUD → `FxState` (9, `6f4cbfd`) ; panneaux Multijoueur → `NetPanelsState` (12, `62695fc`) ; connexion Firebase → `FirebaseAuthState` (5, `f5fd82e`) ; chargements asynchrones → `AsyncLoadState` (5, `a41afd0`) ; joueurs réseau → `NetworkPlayersState` (12, `c9f5f20`, 123 sites) ; projectiles → `ProjectilesState` (8, `b3ce0e7`, 82 sites) ; interpolation de pose → `SimPosesState` (4, `7813417`, 40 sites) ; état tactile → `TouchState` (4, `a8a9978`, 22 sites) ; glissé gizmo → `DragState` (14, `11bc6f3`, 63 sites) ; **mesure de perf → `PerfState`** (6 champs, `e958576`, 110 sites — script étendu à E0615 pour la collision de nom avec l'accesseur `fps()`). 79 champs regroupés, 684/684 tests verts à chaque lot. **Ampleur réelle mesurée** : ~120-175 champs, 71 fichiers concernés — bien plus gros qu'un simple refactor `L`. Reste par lots séparés (IA scène, undo/redo, script Lua...) ; `build_ui` (0 test) reste le point le plus risqué à toucher |

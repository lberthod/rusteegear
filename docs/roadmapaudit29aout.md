...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | 🔶 3 lots faits, méthode : chaque lot vérifié par build + 693 tests + capture d'écran réelle (`screencapture`), pas seulement par compilation. Lot 1 (`bdc1b80`) : 3 modales de confirmation → `confirmation_modals(...)`. Lot 2 (`82ac99c`) : fin de manche/overlay tactile/widgets HUD → `end_of_round_and_hud_widgets(...)`. Lot 3 (`a4d3101`) : mini-carte/collectibles/bannières de Play → `play_overlays(...)`. Les 3 captures faites en mode Édition (rendu intact à chaque fois) — le chemin Play lui-même reste non vérifiable visuellement : tentative d'automatiser un clic sur ▶ Play via `osascript`/System Events refusée (permission Accessibilité macOS non accordée à cette session, pas accordable par elle-même). Code déplacé à l'identique à chaque lot, aucun changement de logique. Reste le gros du corps (barre d'outils ~208 l., bloc panneaux ~660 l., aperçu mobile/cadre téléphone) — nettement plus couplé, chaque lot suivant à traiter avec la même prudence |

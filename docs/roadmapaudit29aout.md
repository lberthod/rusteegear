...
| 2.3 | Regrouper `AppState` en sous-structs par domaine | dette | 🔶 Lot 1/N fait (`6f4cbfd`) : cluster FX/HUD (9 champs) → `FxState`, 120 sites. Lot 2/N fait (`62695fc`) : panneaux Multijoueur (12 champs) → `NetPanelsState`, 34 sites — incident en route (script rejoué sur JSON périmé, corruption de `frame.rs` détectée au diff avant commit, restaurée et refaite proprement). Lot 3/N fait (`f5fd82e`) : connexion Firebase (5 champs) → `FirebaseAuthState`, 18 sites, cluster confiné aux mêmes 3 fichiers que le lot 2. 684/684 tests verts à chaque lot. **Ampleur réelle mesurée** : ~120-175 champs, 71 fichiers concernés — bien plus gros qu'un simple refactor `L`. Reste par lots séparés (état UI éditeur, gameplay, session réseau bas niveau...) ; `build_ui` (0 test) reste le point le plus risqué à toucher |

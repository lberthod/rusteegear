...
| # | Action | Ferme | Statut |
|---|---|---|---|
| 2.1 | Découper `build_ui` (`src/editor/mod.rs:1362-2744`, 1382 lignes, 0 test dessus) par panneau, comme déjà fait pour `windows.rs` / `hud.rs` / `export.rs` | dette | ✅ **Fait, 6/6 lots** (`bdc1b80`, `82ac99c`, `a4d3101`, `5d4eabb`, `5fca41f`, `8cacc0f`) : `confirmation_modals`, `end_of_round_and_hud_widgets`, `play_overlays`, `toolbar`, `inspector_panel` (636 lignes, le plus gros), `play_area_and_in_game_hud` (dernier morceau — zone de jeu, cadre téléphone, HUD in-game). `build_ui` passe de **1382 à ~337 lignes** — reste un simple orchestrateur qui enchaîne des fonctions autonomes, plus une God function. Méthode systématique sur les 6 lots : build + clippy + 693 tests + capture d'écran réelle (`screencapture`) à chaque fois, pas seulement compilation — plusieurs captures ont déclenché spontanément du contenu de lots précédents (modale autosave, Inspecteur), validant plusieurs extractions à la fois. Deux incidents de script détectés par `cargo clippy`/diff avant tout commit (jamais rien de cassé poussé). Seule limite : le chemin Play au clic reste non vérifiable visuellement (permission Accessibilité macOS non accordable par cette session) — risque résiduel identique à tout refactor mécanique vérifié par compilateur+tests |
...
| 2.3 | Regrouper `AppState` en sous-structs par domaine | dette | 🔶 13 lots faits : FX/HUD → `FxState` (9, `6f4cbfd`) ; panneaux Multijoueur → `NetPanelsState` (12, `62695fc`) ; connexion Firebase → `FirebaseAuthState` (5, `f5fd82e`) ; chargements asynchrones → `AsyncLoadState` (5, `a41afd0`) ; joueurs réseau → `NetworkPlayersState` (12, `c9f5f20`, 123 sites) ; projectiles → `ProjectilesState` (8, `b3ce0e7`, 82 sites) ; interpolation de pose → `SimPosesState` (4, `7813417`, 40 sites) ; état tactile → `TouchState` (4, `a8a9978`, 22 sites) ; glissé gizmo → `DragState` (14, `11bc6f3`, 63 sites) ; mesure de perf → `PerfState` (6, `e958576`, 110 sites, script étendu à E0615) ; historique d'édition → `EditHistoryState` (3, `1cca905`, 13 sites) ; génération IA → `AiGenState` (6, `b522b3b`, 13 sites) ; **interpréteur Lua → `ScriptingState`** (5 champs, `19623ed` — variantes natif/wasm32 mutuellement exclusives par `#[cfg]`, préservées à l'identique ; vérifié sur les deux cibles, ce qui a permis de trouver un bug wasm32 pré-existant sans rapport, cf. 3.4). 93 champs regroupés, 684/684 tests verts à chaque lot. **Ampleur réelle mesurée** : ~120-175 champs, 71 fichiers concernés — bien plus gros qu'un simple refactor `L`. Reste quelques champs isolés sans regroupement naturel évident ; `build_ui` (0 test) reste le point le plus risqué à toucher |
| 2.4 | Ajouter des tests sur `src/editor/menus.rs` (765 lignes, 0 test) | qualité | ✅ `90982bb` — pas de harnais `egui` dans ce projet, donc extrait les 2 vraies décisions enfouies dans les closures de menu vers des fonctions pures testées : `MobileControls::toggle_joystick`/`toggle_dpad` (exclusivité joystick/pavé, `scene/mobile.rs`, 6 tests) et `project::is_manifest_path` (classement manifeste vs scène du sélecteur Ouvrir…, 3 tests). Le reste de `menus.rs` est du câblage UI direct sans logique à extraire, reste non testé (vérification manuelle uniquement) |

...
- **2026-07-28** — Premier code Phase 0 : `src/xr/mod.rs` (instance + system
  ...
  de branche.
- **2026-09-24** — Le squelette `src/xr/mod.rs` / feature `xr` de juillet n'a
  jamais été fusionné dans `main` ; il est **remplacé** par le spike Phase 0
  complet de [roadmapExportVRQuest24septembre.md](roadmapExportVRQuest24septembre.md)
  (feature `vr`, `src/xr/hello.rs` : Vulkan créé par OpenXR puis importé dans
  wgpu, swapchain stéréo, cubes en `STAGE` ; APK via `packaging/build_quest.sh`).
  Un Quest 3 est désormais disponible pour tester. Ce document reste la vision
  MR passthrough (portail, araignée IK) ; la roadmap du 24 septembre porte
  l'export VR générique, premier jeu porté : Rivière.

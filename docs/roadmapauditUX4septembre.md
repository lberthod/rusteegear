...
- [x] 2.6 `safe_area` activé par défaut avec vraies marges système, écran maintenu allumé (M-03) — partiel : safe_area activé dans la scène livrée (marge en %, pas les insets système), wake lock sur le web et `KEEP_SCREEN_ON` sur Android (`397ee93`) ; iOS `idleTimerDisabled` posé au démarrage (vérifié par `cargo check --target aarch64-apple-ios`)
...
- [x] 5.6 Spectateur actif : vaincu en ligne, la caméra suit un allié vivant, Espace passe au suivant, la bannière nomme l'allié suivi (P-05) — la réapparition reste une décision de game design (mort définitive par manche dans le GDD), à trancher au playtest

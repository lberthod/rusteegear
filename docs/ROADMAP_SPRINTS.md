...
> **Documents satellites (décision actée, Phase D/Sprint 5 de [sprintreflecion.md](sprintreflecion.md#phase-d))** :
> `sprint10audit.md` (écarts gameplay/GDD), `sprintoptimation3daudit10h.md` (audit perf 3D) et
> `sprintreflecion.md` (validation/déploiement post-audit) **restent des plans satellites**, pas
> renumérotés dans ce fichier — ils couvrent un périmètre disjoint (parité GDD, perf, rollout) de
> la feuille de route moteur ci-dessous et sont déjà cross-référencés par nom depuis ce document
> (ex. § Cadrage produit plus bas) plutôt que fusionnés dans la numérotation de sprints A→S. Les
> anciens documents d'audit qu'ils remplacent (`AUDIT.md`, `AUDIT_MMORPG.md`, `HANDOFF.md`, etc.)
> ont été supprimés le 18 juillet 2026 (commit `cb64951`) après consolidation de leur contenu utile
> dans ces satellites et dans `GDD_MMORPG.md` §14 — rien n'a été perdu sans reprise ailleurs.
...
## 📝 Cadrage produit — prochaines étapes après le cœur de boucle (`sprintreflecion.md` Phase C, Sprint 2)

> Note de cadrage, pas une décision — préparée pendant que la Phase C de `sprintreflecion.md`
> tourne en parallèle du reste. Ne bloque rien, ne planifie aucun sprint.

**Recoupement Contrat du jour / feuille de route moteur** : le catalogue de contrats (GDD §3.4 —
*Nuit blanche*, *À l'aube juste*, *La lande garde ses morts*, *Main de braise*, *Le troupeau compte
sur vous*, *Sobriété*) est vérifié côté serveur avec des compteurs déjà livrés par la Phase F
réseau de ce roadmap (assists de réanimation, frags par source, horodatage de manche, PV du
convoi, `WeaponPickup`) — Sprint 9 de `sprint10audit.md` (terminé 2026-07-18) réutilise déjà cette
discipline. Point de vigilance pour éviter un travail dupliqué : toute extension future du
catalogue de contrats doit continuer à s'appuyer sur ces compteurs réseau existants (avec le même
repli anti-triche que `PlayerClass::from_u8`, jamais de panic serveur sur une valeur hors table),
plutôt que de faire naître un second système de télémétrie parallèle dans une phase moteur
ultérieure (K→Q).

**Rouvrir le périmètre exclu du GDD §12 (artisanat / économie joueur-joueur / guildes) une fois le
cœur de boucle stable — éléments pour la discussion, pas une décision** :

Pour :
- Le contrat du jour et les paliers de niveau ont un plafond assumé (GDD §12 : « le contrat du jour
  est le plafond de la rétention ») — sans profondeur supplémentaire, risque d'essoufflement une
  fois tous les paliers débloqués.
- Les comptes/XP/classement (Firebase) existent déjà et pourraient porter un inventaire ou une
  monnaie sans nouvelle brique d'authentification.
- Une version asynchrone (pas de PvP marchand, pas d'échange temps réel) ne casserait pas
  nécessairement la promesse « jeu coop de poche ».

Contre :
- Chaque exclusion du §12 protège explicitement « un jeu coop de poche qu'une personne seule peut
  comprendre, maintenir et faire évoluer de bout en bout » — artisanat/économie multiplient la
  surface de test et de maintenance solo.
- Nouvelle classe de vulnérabilités (duplication d'objets, farming) alors que la Phase F de
  `sprintreflecion.md` vient tout juste de fermer les trous anti-triche existants.
- 14 phases (A→O) de `sprintreflecion.md` restent à clore ; ouvrir un chantier économie avant
  retarderait la Phase L (playtest) et la Phase M (déploiement).
- Les guildes « au-delà du chat de salon » supposent un état persistant entre salons — contredit
  directement « pas de monde persistant partagé entre salons » (§12) : impliquerait de revoir une
  hypothèse d'architecture, pas juste d'ajouter une fonctionnalité.

...
mod tests {
    ...
    #[test]
    // Flaky historique (constaté le 18/07/2026 : ~60-80 % d'échec sur HEAD non
    // modifié) : le budget de frames ne suffisait pas toujours au missile homing
    // pour vider les salles — sensible au tirage d'arme et aux trajectoires.
    // Réactivé le 2026-08-29 (roadmap post-audit, vague 1.2) après 35/35 passages
    // verts (isolé et sous charge, aux côtés du reste de la suite `--ignored`) :
    // les changements ultérieurs (attack_range/cooldown/windup figés ci-dessous,
    // budget porté à 400 frames) ont visiblement éliminé la source de flakiness.

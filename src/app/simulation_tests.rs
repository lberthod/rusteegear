...
fn mmorpg_creature_wander_crosses_the_west_hill_band_without_getting_stuck() {
...

    {
        ...
        let spawn_h = mmorpg_terrain_local_height(spawn_x / (2.0 * half), spawn_z / (2.0 * half));
    ...
    for step in 0..STEPS {
        ...
        if pos.x >= band_x_lo_m && pos.x <= band_x_hi_m {
            entered_band = true;
            let expected_h = mmorpg_terrain_local_height(pos.x / world_size, pos.z / world_size);
            let y = pos.y;
...
fn mmorpg_creatures_never_teleport_nor_snap_turn() {
    ...
    for step in 0..(20 * 60) {
        ...
        for (k, &i) in creatures.iter().enumerate() {
            ...
            let d_xz =
                (glam::Vec2::new(pos.x, pos.z) - glam::Vec2::new(prev_pos.x, prev_pos.z)).length();
            assert!(
                d_xz <= max_step,
...
/// `bone(nom, dx, dy, dz)` : les directions poussées par le script atterrissent
/// dans `SceneObject::bone_dirs` après le pas, et sont réécrites à chaque pas
/// (vides dès que le script cesse d'en pousser).
#[test]
fn bone_directions_pushed_by_a_script_land_on_the_object_each_tick() {
    let mut app = AppState::new();
    app.scene = crate::scene::Scene::default();
    app.scene.objects.push(SceneObject {
        name: "Pantin".into(),
        script: "if time < 0.05 then bone('UpperArm.L', 0, 1, 0); bone('Hips', 1, 0, 0) end".into(),
        ..Default::default()
    });
    app.playing = true;
    reeduc_tick(&mut app);
    reeduc_tick(&mut app);
    let dirs = &app.scene.objects[0].bone_dirs;
    assert_eq!(dirs.len(), 2, "{dirs:?}");
    assert_eq!(dirs["UpperArm.L"], Vec3::Y);
    for _ in 0..10 {
        reeduc_tick(&mut app);
    }
    assert!(
        app.scene.objects[0].bone_dirs.is_empty(),
        "plus d'appel ⇒ plus d'override"
    );
}
...
/// Attrape-bulles en **mode démo** (aucune pose reçue) : la partie démarre sans
/// calibration, la main droite du mannequin se pilote au joystick, chaque bulle
/// touchée compte, la série monte, et 60 s de temps actif finissent la partie
/// sur un bilan.
#[test]
fn bubbles_demo_mode_catches_bubbles_with_the_joystick() {
    let mut app = AppState::new();
    app.load_reeducation_demo();
    app.playing = true;
    reeduc_tick(&mut app);
    ...
    for _ in 0..2400 {
        ...
        reeduc_tick(&mut app);
        caught = reeduc_var(&app, "rd_caught");
        if caught >= 6.0 {
            break;
        }
    }
    assert!(saw_bubble, "des bulles doivent apparaître");
    assert!(caught >= 6.0, "bulles attrapées au joystick : {caught}");
    assert!(reeduc_var(&app, "rd_points") >= 60.0);
    assert!(app.scene.objects.iter().any(|o| o.name == "Bulle 1"));
    // Fin de partie : 60 s de temps actif.
    let mut n = 0;
    while reeduc_var(&app, "rd_stage") == 2.0 && n < 4000 {
        if let Some(b) = alive_bubble(&app) {
            let hand = object_pos(&app, "Repère wrist_r");
            app.input_state.joy.0 =
                (app.input_state.joy.0 + 0.25 * (b.x - hand.x)).clamp(-1.0, 1.0);
    ...
    assert_eq!(reeduc_var(&app, "rd_stage"), 3.0, "bilan après 60 s");
    let bilan = app.hud_texts.get("bilan").cloned().unwrap_or_default();
    assert!(
        bilan.contains("Partie terminée") && bilan.contains("Record"),
        "{bilan}"
    );
    assert_eq!(reeduc_var(&app, "rd_games"), 1.0);
}

/// Attrape-bulles en **mode caméra** : calibration de 1,8 s de corps visible,
/// puis le poignet droit posé sur une bulle l'attrape ; corps perdu ⇒ pause.
#[test]
fn bubbles_camera_mode_calibrates_then_catches_and_pauses_when_the_body_is_lost() {
    let mut app = AppState::new();
    app.load_reeducation_demo();
    app.playing = true;
    reeduc_tick(&mut app);
    ...
    let mut ticks = 0;
    while reeduc_var(&app, "rd_stage") == 1.0 && ticks < 400 {
        app.set_pose(&flat_pose_from_world(&body));
        reeduc_tick(&mut app);
        ticks += 1;
    }
    ...
    for _ in 0..900 {
        ...
        if let Some(bubble) = alive_bubble(&app) {
            b.iter_mut().find(|(n, _)| *n == "wrist_r").unwrap().1 = (bubble.x, bubble.y);
        }
    ...
    assert!(caught >= 2.0, "bulles attrapées en mode caméra : {caught}");
    ...
    let paused = reeduc_var(&app, "rd_elapsed");
    for _ in 0..30 {
        reeduc_tick(&mut app);
    }
    assert_eq!(
        ...
        paused,
        "chrono figé en pause"
    );
    assert!(paused > elapsed_before);
    app.set_pose(&flat_pose_from_world(&body));
    reeduc_tick(&mut app);
    assert!(
        !app.hud_texts
